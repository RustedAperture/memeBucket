...
use crate::repositories::{
    ...
    images::{
        BulkImageMetadataPatch, ImageRepo, ImageSearchFilters, StoredImage,
        StoredImageSearchResult, UpdateImageMetadataPatch,
    },
};
...
impl<R: BucketRepo> BucketRepo for CachedBucketRepository<R> {
    ...
    async fn rename_bucket(
        ...
        let result = self
            .inner
            .rename_bucket(bucket_id, owner_user_id, new_name)
            .await?;
        self.invalidate_bucket_with_token(bucket_id).await;
        Ok(result)
    }
    ...
    async fn find_accessible_by_name_folded(
        ...
        self.inner
            .find_accessible_by_name_folded(requester_user_id, name)
            .await
    }
    ...
    async fn set_share_token(
        ...
        let result = self
            .inner
            .set_share_token(bucket_id, owner_user_id, token)
            .await?;
        self.invalidate_bucket_with_token(bucket_id).await;
        Ok(result)
    }
    ...
    async fn subscribe_user_to_bucket(
        ...
        let result = self
            .inner
            .subscribe_user_to_bucket(user_id, bucket_id)
            .await?;
    ...
    async fn unsubscribe_user_from_bucket(
        ...
        let result = self
            .inner
            .unsubscribe_user_from_bucket(user_id, bucket_id)
            .await?;
        self.user_subscribed.invalidate(&user_id).await;
        Ok(result)
    }
    ...
    async fn list_subscribed_for_user(
        ...
        let result = self
            .inner
            .list_subscribed_for_user(subscriber_user_id)
            .await?;
        self.user_subscribed
            .insert(subscriber_user_id, result.clone())
            .await;
        Ok(result)
    }
    ...
    async fn set_whitelist_enabled(
        ...
        let result = self
            .inner
            .set_whitelist_enabled(bucket_id, owner_user_id, enabled)
            .await?;
        self.invalidate_bucket_with_token(bucket_id).await;
        Ok(result)
    }
    ...
    async fn add_whitelist_user(
        ...
        let result = self
            .inner
            .add_whitelist_user(bucket_id, owner_user_id, username)
            .await?;
        self.whitelist_users.invalidate(&bucket_id).await;
        self.is_whitelisted.invalidate_all();
        Ok(result)
    ...
    async fn remove_whitelist_user(
        ...
        let result = self
            .inner
            .remove_whitelist_user(bucket_id, owner_user_id, username)
            .await?;
        self.whitelist_users.invalidate(&bucket_id).await;
        self.is_whitelisted.invalidate_all();
        Ok(result)
    ...
    async fn list_whitelist_users(
        ...
        let result = self
            .inner
            .list_whitelist_users(bucket_id, owner_user_id)
            .await?;
...
impl<R: ImageRepo> ImageRepo for CachedImageRepository<R> {
    ...
    async fn get_for_owner(
        ...
        let result = self
            .inner
            .get_for_owner(owner_user_id, bucket_id, image_id)
            .await?;
    ...
    async fn delete_for_user(
        ...
        let result = self
            .inner
            .delete_for_user(owner_user_id, bucket_id, image_id)
            .await?;
        self.list_for_bucket.invalidate_all();
        self.get_for_owner
    ...
    async fn update_notes(
        ...
        let result = self
            .inner
            .update_notes(owner_user_id, bucket_id, image_id, notes)
            .await?;
        self.list_for_bucket.invalidate_all();
        self.get_for_owner
            .invalidate(&(owner_user_id, bucket_id, image_id))
    ...
    async fn update_metadata_partial(
        ...
        let result = self
            .inner
            .update_metadata_partial(owner_user_id, bucket_id, image_id, patch)
            .await?;
        self.list_for_bucket.invalidate_all();
        self.get_for_owner
            .invalidate(&(owner_user_id, bucket_id, image_id))
    ...
    async fn update_metadata_bulk(
        ...
        let result = self
            .inner
            .update_metadata_bulk(owner_user_id, bucket_id, patch)
            .await?;
        self.list_for_bucket.invalidate_all();
        for id in &patch.image_ids {
            self.get_for_owner
                .invalidate(&(owner_user_id, bucket_id, *id))
                .await;
        }
        Ok(result)
    }
    ...
    async fn delete_bulk(
        ...
        let result = self
            .inner
            .delete_bulk(owner_user_id, bucket_id, image_ids)
            .await?;
        self.list_for_bucket.invalidate_all();
        for id in image_ids {
            self.get_for_owner
    ...
    async fn move_bulk(
        ...
        let result = self
            .inner
            .move_bulk(owner_user_id, bucket_id, image_ids, new_bucket_id)
            .await?;
        self.list_for_bucket.invalidate_all();
        for id in image_ids {
            self.get_for_owner
                ...
                .await;
            self.get_for_owner
                .invalidate(&(owner_user_id, new_bucket_id, *id))
                .await;
        }
        Ok(result)
    }
    ...
    async fn move_to_bucket(
        ...
        let result = self
            .inner
            .move_to_bucket(owner_user_id, bucket_id, image_id, new_bucket_id)
            .await?;
        self.list_for_bucket.invalidate_all();
        self.get_for_owner
            .invalidate(&(owner_user_id, bucket_id, image_id))
            .await;
        self.get_for_owner
            .invalidate(&(owner_user_id, new_bucket_id, image_id))
            .await;
        Ok(result)
    }
}
